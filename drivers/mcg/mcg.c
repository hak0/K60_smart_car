...
unsigned char pll_init(clk_option opt)
{
    ...
    if(opt  != PLLUSR ) //自定义模式，直接加载全局变量mcg_div的值
    {
        ...
        case PLL170:
            ...
            break;
        case PLL180:
            mcg_div.prdiv       = 9;  //50 * (12+24) /10 = 180
            mcg_div.vdiv        = 12;  // 50 * 54 /15 = 180
            break;
        case PLL200:
            mcg_div.prdiv       =  11; //12;    50*48/12=200
            mcg_div.vdiv        = 24;//28;  //50u/(PRDIV+1)*(VDIV+24) = 50 * 52 /13 = 200
            break;
        case PLL225:
            mcg_div.prdiv       = 11;
            mcg_div.vdiv        = 30;
        ...
        else if(opt <= 5 * MAX_BUS_CLK)   mcg_div.bus_div = 4;    // bus  = MCG/5    这里提示警告，不过没关系
    ...
    //mcg_div.flex_div = mcg_div.flash_div;                       // flex   = flash
