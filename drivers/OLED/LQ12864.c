...
G    电源地
3.3V 接3.3V电源
D0   PC18  PORTA_PA14     PA4
D1   PC16  PORTA_PA15     PA5
RST  PC14  PORTA_PA16     PA6
DC   PC12  PORTA_PA17     PA7
CS   已接地，不用接
============================================
OLED电源使用5V。   
----------------
...
void oled_init()
{
   ...
   LCD_Fill(0x00);//亮屏
   delayms(100);  
   //LCD_Print(33,4,"红树伟业");   //汉字字符串显示   
   //LCD_P6x8Str(80,7,"2015-03");     //英文字符串显示
   delayms(100);      
   //LCD_CLS();    
   //LCD_P6x8Str(80,5,"2014-05");  
}
void LCD_WrDat(byte data)
{
  byte i=8;
  //LCD_CS=0;;
//  GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(7));;;;
//  asm("nop"); 
//  GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(4));;;;
//  asm("nop");    
//  while(i--)
//  {
//    if(data&0x80){GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(5));;;;}
//    else{GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(5));;;;}
//    GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(4)); 
//    asm("nop");;;;
//   //asm("nop");            
//    GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(4));;;;;    
...
//  }
  ...
  while(i--)
  {
    if(data&0x80){GPIOC_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(16));;;;}
    else{GPIOC_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(16));;;;}
    GPIOC_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(18)); 
    asm("nop");;;;
   //asm("nop");            
    GPIOC_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(18));;;;;    
...
//  while(i--)
//  {
//    if(cmd&0x80){GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(5));;;;;}
//    else{GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(5));;;;;;}
//    GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(4));;;;;
//    asm("nop");;;;
//   //asm("nop");             
...
//  } 	
  ...
  while(i--)
  {
    if(cmd&0x80){GPIOC_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(16));;;;;}
    else{GPIOC_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(16));;;;;;}
    GPIOC_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(18));;;;;
    asm("nop");;;;
   //asm("nop");             
...
//  PORTA_PCR4=(0|PORT_PCR_MUX(1));
//  PORTA_PCR5=(0|PORT_PCR_MUX(1)); 
//  PORTA_PCR6=(0|PORT_PCR_MUX(1));
//  PORTA_PCR7=(0|PORT_PCR_MUX(1)); 
//  //设置PORTA pin14,pin15为输出方向;pin16,pin17为输入方向
//  GPIOA_PDDR=GPIO_PDDR_PDD(GPIO_PIN(4)|GPIO_PIN(5)|GPIO_PIN(6)|GPIO_PIN(7));
//  GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(4));
//  //LCD_CS=1;	//预制SLK和SS为高电平   	
//  GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(6));
//  LCD_DLY_ms(50);
//  GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(6));

  //设置PORTC pin18,pin16为GPIO口 
  PORTC_PCR18=(0|PORT_PCR_MUX(1));
  PORTC_PCR16=(0|PORT_PCR_MUX(1)); 
  PORTC_PCR14=(0|PORT_PCR_MUX(1));
  PORTC_PCR12=(0|PORT_PCR_MUX(1)); 
  //设置PORTC pin18,pin16为输出方向;pin14,pin12为输入方向
  GPIOC_PDDR=GPIO_PDDR_PDD(GPIO_PIN(18)|GPIO_PIN(16)|GPIO_PIN(14)|GPIO_PIN(12));
  GPIOC_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(18));
  //LCD_CS=1;	//预制SLK和SS为高电平   	
...
void Dis_numint(byte x,byte y,int asc) 
{
  ...
  if(asc>=0)
  { ch2[0] = '+';
    ch2[1] = asc/10000+0x30;	
    ch2[2] = asc%10000/1000+0x30;
    ch2[3] = asc%1000/100+0x30;
    ch2[4] = asc%100/10+0x30;
    ch2[5] = asc%10+0x30;
    ch2[6] = '\0';//字母占一个字节
    LCD_P6x8Str(x , y , ch2);	//显示字母
  }
